...
- **Portable download:** `NUMBSKULL-v….-portable.zip` has its own Python, every library, the pretrained model and the panel. Extract it and double-click `NUMBSKULL.bat`, with nothing to install. `build_portable.py` builds it. See [Setup](README.md#setup).
