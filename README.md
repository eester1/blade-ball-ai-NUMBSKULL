...
- **Ball lost while you're not targeted:** the camera waits; it doesn't search (`CAMERA_IDLE_SEARCH`). Searching found the ball only 37% of the time and didn't improve survival, while in busy fights it chained into circling. The rest of this point describes the search if it's switched back on.
- **Ball lost (search, if switched on):** turn toward the side it was last heading, to find it again — after 2 frames if it was heading off the edge of the screen, otherwise after ~half a second (a small far ball is often just missed for a few frames). While the ball isn't after you, that search gets about **1 second** of sweeping in total (`CAMERA_SEARCH_MAX_S`), and only a steadily tracked ball resets it. It used to be allowed 6 seconds, restarted by every brief detection, and after a block, with the ball flown off far and small, the camera spun round in full circles looking for it. While you're targeted, the search keeps going until the ball is found. In mouse mode searches drag 1.5x faster than normal turns (`CAMERA_SEARCH_SPEED`).
