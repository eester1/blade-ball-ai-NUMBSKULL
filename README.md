...
### Easiest: the portable download (nothing to install)
...
| `white_split_sat_max` | On a pale sky-and-sand map the background is as bright as the ball (~235), so brightness can't split them. If the brightness split finds nothing, failed white blobs are re-examined for **nearly colourless** pixels only (saturation ≤ 10): the background is faintly tinted (12–45), the ball isn't (3–5). On one such map this found the ball in 20% of the frames it had missed. Across frames from 43 runs, it adds a candidate in about 2% of them, and those were the real ball when checked by eye |
...
- **Switching it on and off:** the **Learned block timing** tick box (unticked every time the panel opens) (`python play_live.py model.joblib --learned-block`). When the AI starts, the output box shows either `Learned block timing: taps within 39 ball radii (3D), learned … from … targetings`, or that nothing has been learned yet (it then uses the built-in rules). Unticked, it uses the built-in rules; nothing else is affected, so you can compare the two and go back at any time.
...
- **You're targeted and no ball is steadily tracked in view:** turn right away to find it — unless a tracked ball was in view, well inside the screen, in the last 0.25 s (`CAMERA_TARGET_GRACE_S`). When the ball turns on you it changes colour and speeds up, and often isn't detected for a few frames. In the logs, of 15 targetings where the ball wasn't on screen when it arrived (9 of them deaths), 11 had it in view less than 0.35 s before, and the camera then spun away from it. A ball heading off the edge of the screen is still searched for at once, toward that side. (A steadily tracked ball is always kept in view instead, whatever color it reads as — an earlier version searched whenever the tracked ball wasn't red, and in live testing that turned the camera away from the real ball.) When the ball targets you, Blade Ball tints your own character red — `track_ball.self_highlight_score` measures that in a fixed region around your character, so it works even when the ball is behind the camera, which is exactly when you'd otherwise die without seeing it.
...
- **Ball lost while you're not targeted:** once it's been missing for 1 second, one **slow** sweep: about 150° a second, one full turn in about 2.5 seconds, at most one every 7 seconds (`CAMERA_IDLE_*`). It pauses as soon as anything ball-like comes into view, and carries on if that was nothing. A normal-speed turn goes all the way round in under a second (measured from live frames), so a faster sweep showed the ball for only a few frames per turn, too few to recognise it, and spun straight past it. Each round starts fresh, so at the start of a round it looks for the ball straight away. Both extremes went wrong in live play. Searching every time the ball was lost chained short searches into the camera circling in busy fights. Not searching at all meant the ball, usually out of view when a round starts, was found in only 12% of frames instead of about 60%, and the AI barely moved, since the model only acts when it sees the ball.
