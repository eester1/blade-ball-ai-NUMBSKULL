...
- **Block timing is decided by distance to your character.** The model gets block timing wrong in both directions: it once tapped with the ball ~1.1 s away (block used up, died), and right at contact it often *doesn't* want to block, since in the recordings you'd already pressed earlier. So a block only goes through once the ball is within `BLOCK_MAX_CHAR_DIST` (200 px) of your character on screen — whatever direction it comes from, a ball reaching you ends up at your character's spot; every successful live block was 121–191 px away and every contact 113–195 px. Once that close, it blocks if the model wants to **or** if you're targeted and the ball is red. Raise the distance if it blocks too late, lower it if too early.
